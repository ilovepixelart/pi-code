...
- Reads the plugin cache at `~/.claude/plugins/cache`, active per `enabledPlugins` in user settings only: a checked-out repo cannot flip which code-bearing plugins run, so project settings never toggle them. The version that loads is the one `~/.claude/plugins/installed_plugins.json` records (a user-scope install first); with no usable record, the newest version directory not marked `.orphaned_at`. Cache directories can be named by commit sha, so the name alone does not say which is newest.
