...
- Skills: the plugin's `skills/` directory is always scanned, and any directory the manifest's `skills` names is loaded alongside it rather than replacing it. A plugin shipping exactly one skill can put `SKILL.md` at its root instead.
