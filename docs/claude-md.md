...
- Resolves `@path` imports pi's native loader skips (4-hop depth, budget-capped). An import that resolves outside what its importing file may read is refused rather than approved through a dialog, and the refused paths are listed under `## Imports not loaded (@)` so the gap is visible. The list names the path the importing file wrote, never where a symlink pointed, and reports a target that does not exist the same as one that does, so it carries no information about the filesystem that the importing file did not already have. Claude instead asks once per project and loads what you allow, so its worktree recipe (`@~/.claude/my-project-instructions.md` in a project `CLAUDE.md`) does not load here.
