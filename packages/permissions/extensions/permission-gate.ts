...
export default function permissionGate(pi: ExtensionAPI) {
  ...
  pi.on('tool_call', async (event, ctx) => {
    try {
      ...
      const value = surface === 'bash' ? (input.command as string) : ((input.path ?? input.file_path) as string)
      ...
      const choice = await ctx.ui.select(`⚠️ Permission required (${surface}):\n\n  ${value}\n\nAllow?`, ['Yes (once)', `Yes, allow "${pattern}" for this session`, `Yes, always allow "${pattern}" (saved to project)`, 'No (block)'])
