 ...
 *
 * Config merge order (later wins): ~/.pi/agent/mcp.json then .pi/mcp.json.
...
export function loadConfig(cwd: string): Record<string, ServerConfig> {
  return loadConfigFrom([path.join(os.homedir(), '.pi', 'agent', 'mcp.json'), path.join(cwd, '.pi', 'mcp.json')])
}
...
export default async function mcpExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand('mcp', {
    ...
    handler: async (_args, ctx) => {
      if (status.size === 0) {
        ctx.ui.notify('No MCP servers configured. Add them to ~/.pi/agent/mcp.json or .pi/mcp.json', 'info')
        return
      }
