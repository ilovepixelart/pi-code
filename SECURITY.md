...
- **A command's `allowed-tools` is enforced per tool, not per argument.** Claude scopes a grant to arguments, so `Bash(git add:*)` pre-approves exactly those commands. pi's active-tool list has no argument dimension, so the scope is dropped and the base tool is granted for the turn: a command declaring `Bash(git add:*)` can run any bash command. Dropping the whole entry instead, which is what happened before, left such a command running with no tools at all rather than restricted ones. Read a command's `allowed-tools` as naming which tools it may reach, not which arguments it may pass them.
