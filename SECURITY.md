...
- **The consent prompt reports the wrong source.** It shows `(unknown)` unless the project agents came from `.pi/agents`, and the agent name is interpolated without stripping control characters, so a crafted name can forge the provenance line. Agent discovery itself now stops at the project root (`.git` or `package.json`); a project with neither marker is searched only in the working directory.
