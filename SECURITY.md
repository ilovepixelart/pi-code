...
- **skills are not trust-gated.** Their path handling is safe (a fixed `.claude/skills` path, no filename or content reaches a command), but the directory is handed to pi's loader without a trust check. The residual risk is pi's loader surfacing untrusted skill descriptions to the model. Commands are gated: a project `.claude/commands` body can run shell (`` !`cmd` ``) and inline files (`@path`), so project commands register only once the project is approved.
