...
- **Checkpoints copy the working tree into `~/.pi/agent/checkpoints` and are never garbage-collected.** Each session gets a bare shadow repo holding full snapshots of the tracked and non-ignored-untracked files at every turn. `.gitignore` is honored, so the usual `.env` case is excluded, but any non-ignored secret or large asset is copied under the home directory and outlives the session; there is no automatic cleanup. Delete stale repos under that directory manually.
