  ...
  const boundRunner =
    ...
    (hook, payload, ms) => {
      ...
      const dispatch = (onChild?: (kill: () => void) => void): Promise<HookRunResult> => {
        ...
        return runHookCommand(hook.command, merged, ms, projectDir, hook.args, onChild, hook.shell, hook.pluginRoot !== undefined && hook.pluginDataDir !== undefined ? { root: hook.pluginRoot, dataDir: hook.pluginDataDir } : undefined)
      }
