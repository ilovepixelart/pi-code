 ...
 */
const layoutOverlay = (todos: Todo[], budget: number): OverlayLayout => {
...
class TodoOverlay {
	...
	private renderWidget(theme: Theme, width: number): string[] {
		...
		const lines = [
			truncate(theme.fg(headingColor, `${headingIcon} Todos (${completed}/${todos.length})`)),
		];
...
export default function (pi: ExtensionAPI) {
	...
	const fail = (action: TodoAction, error: string) =>
		toolMessage(`Error: ${error}`, { action, todos: [...todos], nextId, error });
	...
	const handleList = () =>
		ok(
			"list",
			todos.length ? todos.map((t) => `${listMark(t.status)} #${t.id}: ${t.text}`).join("\n") : "No todos",
		);
